...
function openCreateCourseModal(url) {    
    $.ajax({
        url: url,
        dataType: 'json',
        processData: false,
        ...
        success: function(responseObj) {
            console.log(responseObj);
              if (responseObj.status == "error") {   
                 ...
                 jQuery('#modal-add-course').html(responseObj.view).modal('show', { backdrop: 'static' });
                 $('#Addform').parsley();
              }
        },
    ...
    $.ajax({
        ...
        success: function(responseTxt, textStatus, jqXHR) {
            if (responseTxt.status == "success") {
                ...
                $('#coursesDT').DataTable().ajax.reload();
